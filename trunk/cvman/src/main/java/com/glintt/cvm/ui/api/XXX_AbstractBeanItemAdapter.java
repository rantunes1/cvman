...
public abstract class XXX_AbstractBeanItemAdapter<POJO extends AbstractPojo> extends BeanItem<POJO> implements FormFieldFactory {

    private final DefaultFieldFactory factory = new DefaultFieldFactory() {
    };

    private Map<Property, XXX_BuildableObjectProperty> propertyCache;

    public XXX_AbstractBeanItemAdapter(POJO bean) {
        super(bean);
        initializeItemProps();
    }

    public XXX_AbstractBeanItemAdapter(POJO bean, Collection<?> propertyIds) {
        super(bean, propertyIds);
        initializeItemProps();
    }

    public XXX_AbstractBeanItemAdapter(POJO bean, String[] propertyIds) {
        super(bean, propertyIds);
        initializeItemProps();
    }
    ...
    protected abstract Field createField(Item item, String propertyId, XXX_BuildableObjectProperty property, Component uiContext);

    @Override
    public XXX_BuildableObjectProperty getItemProperty(Object id) {
        Property property = super.getItemProperty(id);

        if (this.propertyCache == null)
            this.propertyCache = new HashMap<Property, XXX_BuildableObjectProperty>();

        XXX_BuildableObjectProperty buildableObjectProperty = this.propertyCache.get(property);
        if (buildableObjectProperty == null) {
            buildableObjectProperty = new XXX_BuildableObjectProperty(property);
    ...
    public Field createField(Item item, Object propertyId, Component uiContext) {
        XXX_BuildableObjectProperty property = (XXX_BuildableObjectProperty) item.getItemProperty(propertyId);
    ...
    protected RepaintRequestListener getRepaintRequestListener(final Field field, final XXX_BuildableObjectProperty property) {
